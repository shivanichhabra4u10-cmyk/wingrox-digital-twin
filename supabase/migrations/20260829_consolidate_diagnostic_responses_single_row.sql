-- Consolidate diagnostic responses into a single row per participant/questionnaire.
...
-- Backfill from old per-question response rows if they exist.
...
select
  ...
  max(dqr.created_by) as created_by,
  max(dqr.updated_by) as updated_by
from public.diagnostic_question_responses dqr
group by dqr.questionnaire_id, dqr.participant_id
...
drop table if exists public.diagnostic_question_responses;
